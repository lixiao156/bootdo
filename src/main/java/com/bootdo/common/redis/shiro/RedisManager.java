...
public class RedisManager {
    ...
    private String host = "192.168.150.128";

    @Value("${spring.redis.port}")
    private int port = 6391;

    // 0 - never expire 设置 key的存活时间
    private int expire = 10000;
    ...
    public byte[] set(byte[] key, byte[] value) {
        Jedis jedis = jedisPool.getResource();
        try {
            //数组转化为了流  两者虽然是  字节数组 但是 两个都是Object类型转化而来的
            jedis.set(key, value);
            // 设置时间
            if (this.expire != 0) {
