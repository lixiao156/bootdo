...
//		 <option value="announcement">公告</option>	
	...
	public String shownews(@RequestParam(value = "id", required = false, defaultValue = "0")int id, Model model) {
		model.addAttribute("shownewsInfo", newsService.get(Long.valueOf(id)));
		HashMap map = new HashMap<String,Object>();
		String url = "http://localhost/shownews.html"+id;
		int times = statisticService.RedisSearch(url,id);
		StatisticDo statisticDo = new StatisticDo();
		statisticDo.setTimes(times);

		model.addAttribute("addCliked",statisticDo);
		//补充页面的信息查询
		setPageInfo(model);
